...
export default function AplicacionesAceptadas() {
    ...
    const fetchPolizas = async () => {
      try {
        const resp = await fetch(
          `http://localhost:33761/api/aplicaciones/usuarios/aplicaciones-aceptadas-usuario/${id_usuario}`
        );
        ...
        const detallesPromises = lista.map((poliza) => {
          return fetch(
            `http://localhost:33761/api/aplicaciones/usuarios/obtener-detalles-aplicacion/${poliza.id_poliza}/${id_usuario}`
          ).then((res) => res.json());
        });
  ...
  return (
    ...
    >
      ...
      <main className="relative flex-1 flex flex-col items-center px-4 z-10 mt-10 md:mt-16">
        
        <div className="max-w-6xl w-full bg-white/90 rounded-2xl shadow-lg p-12 backdrop-blur-md border border-green-100">

          <h1 className="text-3xl md:text-4xl font-extrabold text-green-700 mb-6 text-center md:text-left">
            Aplicaciones Aceptadas para el usuario: {user.nombre_completo}
          </h1>

          {polizasAceptadas.length === 0 ? (
            <div className="text-center text-gray-600 py-10">
              No tiene pólizas aceptadas.
            </div>
          ) : (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-center">
                        <div className="flex flex-col md:flex-row gap-2 justify-center text-center items-center">
                          <Link
                            to={`/global/historial_pagos/${poliza.id_registro_en_poliza}`}
                            state={{ poliza }}
