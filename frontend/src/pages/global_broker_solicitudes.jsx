const aseguradoras = [
  { id_aseguradora: 1, nombre: "Aseguradora Andina" },
  { id_aseguradora: 2, nombre: "Seguros Pacífico" },
];

const solicitudes = [
  {
    id_usuario: 10,
    nombres: "Lucía",
    apellidos: "Mendoza",
    email: "lucia@aseguradora.com",
    rol_solicitado: "Analista",
    estado: "PENDIENTE",
    id_aseguradora: 1,
  },
  {
    id_usuario: 11,
    nombres: "Javier",
    apellidos: "Salazar",
    email: "javier@aseguradora.com",
    rol_solicitado: "Administrador",
    estado: "PENDIENTE",
    id_aseguradora: 1,
  },
  {
    id_usuario: 12,
    nombres: "Paola",
    apellidos: "Rojas",
    email: "paola@aseguradora.com",
    rol_solicitado: "Analista",
    estado: "PENDIENTE",
    id_aseguradora: 2,
  },
  {
    id_usuario: 13,
    nombres: "Marco",
    apellidos: "Lozano",
    email: "marco@aseguradora.com",
    rol_solicitado: "Administrador",
    estado: "APROBADO",
    id_aseguradora: 2,
  },
];

export default function GlobalBrokerSolicitudesEquipo() {
  const idAsegLocal = localStorage.getItem("id_aseguradora");
  const idAseguradora = idAsegLocal ? parseInt(idAsegLocal, 10) : null;

  const aseguradoraActual =
    aseguradoras.find((a) => a.id_aseguradora === idAseguradora) || {
      nombre: "Tu Aseguradora",
    };

  const solicitudesPendientes = solicitudes.filter(
    (s) =>
      s.estado === "PENDIENTE" &&
      (idAseguradora ? s.id_aseguradora === idAseguradora : true)
  );

  const handleAprobar = (solicitud) => {
    alert(
      `Aprobaste la solicitud de ${solicitud.nombres} ${solicitud.apellidos} como ${solicitud.rol_solicitado}`
    );
  };

  const handleRechazar = (solicitud) => {
    alert(
      `Rechazaste la solicitud de ${solicitud.nombres} ${solicitud.apellidos} como ${solicitud.rol_solicitado}`
    );
  };

  return (
    <div
      className="min-h-screen w-screen flex flex-col bg-cover bg-center bg-no-repeat"
      style={{
        backgroundImage:
          "url('https://images.unsplash.com/photo-1521737604893-d14cc237f11d?auto=format&fit=crop&w=1400&q=80')",
      }}
    >
      <div className="bg-white/70 flex flex-col min-h-screen backdrop-blur-sm">
        <header className="flex justify-between items-center px-8 py-4 text-green-700 font-semibold">
          <div className="text-xl">MiSeguroDigital</div>
        </header>

        <div className="px-6 mt-6 text-center">
          <h1 className="text-4xl font-extrabold text-green-700 drop-shadow-sm">
            Solicitudes de Analistas y Administradores
          </h1>
          <p className="text-gray-700 mt-2 text-lg">
            Aseguradora:{" "}
            <span className="font-semibold">{aseguradoraActual.nombre}</span>
          </p>
          <p className="text-gray-600 mt-1">
            Revisa y aprueba o rechaza las solicitudes de acceso al equipo.
          </p>
        </div>

        <main className="flex-1 flex flex-col justify-start items-center px-6 mt-6 mb-6">
          <div className="max-w-5xl w-full bg-white/85 rounded-2xl shadow-xl p-10">
            <table className="min-w-full border text-sm rounded-lg overflow-hidden shadow-sm">
              <thead className="bg-green-50 text-green-900">
                <tr>
                  <th className="border px-3 py-2">ID Broker</th>
                    <th className="border px-3 py-2">Nombre</th>
                  <th className="border px-3 py-2">Email</th>
                  <th className="border px-3 py-2">Rol solicitado</th>
                  <th className="border px-3 py-2 text-center">Acciones</th>
                </tr>
              </thead>
              <tbody>
                {solicitudesPendientes.map((s) => (
                  <tr
                    key={s.id_usuario}
                    className="odd:bg-gray-50 hover:bg-green-50 transition"
                  >
                    <td className="border px-3 py-2 font-medium">
                      {s.id_usuario}
                    </td>
                    <td className="border px-3 py-2">
                      {s.nombres} {s.apellidos}
                    </td>
                    <td className="border px-3 py-2">{s.email}</td>
                    <td className="border px-3 py-2">{s.rol_solicitado}</td>

                    <td className="border px-3 py-2 text-center space-x-3">
                      <button
                        className="px-3 py-1 !bg-green-600 text-white text-xs rounded-lg hover:bg-green-700 transition"
                        onClick={() => handleAprobar(s)}
                      >
                        Aprobar
                      </button>

                      <button
                        className="px-3 py-1 !bg-red-600 text-white text-xs rounded-lg hover:bg-red-700 transition"
                        onClick={() => handleRechazar(s)}
                      >
                        Rechazar
                      </button>
                    </td>
                  </tr>
                ))}

                {solicitudesPendientes.length === 0 && (
                  <tr>
                    <td
                      colSpan={5}
                      className="border px-3 py-4 text-center text-gray-600"
                    >
                      No hay solicitudes pendientes para esta aseguradora.
                    </td>
                  </tr>
                )}
              </tbody>
            </table>
          </div>
        </main>

        <footer className="py-4 text-gray-600 text-sm text-center">
          © 2025 MiSeguroDigital — Global Broker.
        </footer>
      </div>
    </div>
  );
}
