...
export default function HistorialPagosGlobal() {
  ...
  const handleReembolsar = async (id_pago) => {
    const resp=await fetch(
      `http://localhost:33761/api/pagos/admin/reembolsar-pago/${id_pago}`,
      {
        method: "PUT",
      }
    );
    ...
    if(!resp.ok){
      alert("Error al procesar el reembolso: "+(data.message || "Error desconocido"));
      return;
    }

    const resp2=await fetch(
      `http://localhost:33761/api/pagos/admin/eliminar-pago/${id_pago}`,
      {
        method: "DELETE",
      }
    );
    ...
    if(!resp2.ok){
      alert("Error al procesar el reembolso: "+(data2.message || "Error desconocido"));
      return;
    }

    alert("Reembolso procesado exitosamente y eliminación realizada.");
    setPagos((prevPagos) =>
      prevPagos.filter((pago) =>
      ...
      <main className="relative flex-1 flex flex-col justify-center items-center px-4 z-10">
        <div className="max-w-5xl w-full bg-white/80 rounded-2xl shadow-lg p-10 backdrop-blur-md border border-green-100">
          ...
          <p className="text-gray-700 text-sm md:text-base mb-6 text-center md:text-left">
              Aquí puedes revisar todos los pagos realizados para el registro {registryID}
          </p>
