...
create or replace view ViewAplicacionesAceptadasPorUsuario as
select
    -- ? Informacion del usuario
    registro_usuarios.id_usuario,
    -- ? Inforkacino registro
    RegistroDeUsuarioEnPoliza.id_registro_en_poliza,
    -- ? Informacion de la aplicacion
    aplicacionPoliza.id_aplicacion_poliza,
    aplicacionPoliza.fecha_de_aplicacion,
    aplicacionPoliza.estado_actual_aplicacion,
...
from
    AplicacionAPoliza aplicacionPoliza
        ...
        join Aseguradoras aseguradora
             on polizas.id_aseguradora = aseguradora.id_aseguradora
        -- ? Unior con la tabla de registros de polizas
        join RegistroDeUsuarioEnPoliza
             on registro_usuarios.id_usuario = RegistroDeUsuarioEnPoliza.id_usuario
                 and aplicacionPoliza.id_poliza = RegistroDeUsuarioEnPoliza.id_poliza
order by
    -- ? Ordenar por fecha de aplicacion descendente
    aplicacionPoliza.fecha_de_aplicacion desc;

select * from ViewAplicacionesAceptadasPorUsuario;
